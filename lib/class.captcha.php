 ...
 * @author		xagero
 */
class PluginLiteCaptcha extends PluginLightCaptcha_Abstract
{
	// }}}
	// {{{ init
	...
	public function init()
	{
		...
		$this->viewIndexAll = array(
			'overview' => array(
				'title-tab' => __('Overview'),
				'title' => $this->_config['meta']['Name'] . '&nbsp;Overview'
			),
			'settings-symbol' => array(
				'title-tab' => __('Character CAPTCHA Settings','light-captcha'),
				'title' => $this->_config['meta']['Name'] . '&nbsp;Settings'
			),
			'settings-math' => array(
				'title-tab' => __('Mathematical CAPTCHA Settings','light-captcha'),
				'title' => $this->_config['meta']['Name'] . '&nbsp;Settings'
			)
	...
	{
		$myStyleUrl = WP_PLUGIN_URL . '/light-captcha/style/style.css';
        wp_register_style('light-captcha', $myStyleUrl);
		wp_enqueue_style('light-captcha');
	}
...
';
	...
	<img src="'.WP_PLUGIN_URL.'/light-captcha/style/images/reload.png" id="reload_captcha" alt="Captcha Reload" title="'.__("Generate new CAPTCHA","light-captcha").'"/><br />';

	echo '
  <label for="captcha_input">'.__("CAPTCHA Code","light-captcha").'</label>
...
';
	...
	<img src="'.WP_PLUGIN_URL.'/light-captcha/style/images/reload.gif" id="reload_captcha" alt="Captcha Reload" title="'.__("Generate new CAPTCHA","light-captcha").'"/><br />';
	echo '<label for="captcha_input">'.__("Result","light-captcha").'</label>
...
public function thethe_captcha_comment_post($comment) {
	...
	if (empty($_POST['captcha_input']) || $_POST['captcha_input'] == '') {
	   wp_die( __('Error: You have not entered a CAPTCHA text/result. Press your browser\'s back button and try again.', 'light-captcha'));
	}
   	$config_m = $this->config('default-m');
	$config_s = $this->config('default-s');
	...
	if($valid == true) {
	   ...
	   wp_die( __('Error: You have entered a wrong CAPTCHA text/result. Press your browser\'s back button and try again.', 'light-captcha'));
	}
...
';
	...
	echo '
	<label for="captcha_input">'.__("CAPTCHA Code","light-captcha").'<br />
...
';
	...
	<img src="'.WP_PLUGIN_URL.'/light-captcha/style/images/reload.gif" id="reload_captcha" alt="Captcha Reload" title="'.__("Generate new CAPTCHA","light-captcha").'"/><br />';
	echo '
	<label for="captcha_input">'.__("Result","light-captcha").'<br />
	<input id="captcha_input" type="text" name="captcha_input" class="input" /></label>
	</p>
	';
  }
...
function thethe_captcha_register_post($errors) {
	  ...
	  if (empty($_POST['captcha_input']) || $_POST['captcha_input'] == '') {
				$errors->add('thethe_captcha_blank', '<strong>'.__('ERROR', 'light-captcha').'</strong>: '.__('Please Complete the CAPTCHA.', 'light-captcha'));
				return $errors;
	  } 
    $config_m = $this->config('default-m');
	$config_s = $this->config('default-s');
	...
	if($valid == true) {
	 ...
	 $errors->add('thethe_captcha_wrong', '<strong>'.__('ERROR', 'light-captcha').'</strong>: '.__('That CAPTCHA was incorrect. Make sure you have not disabled cookies.', 'light-captcha'));
	}
	unset($_SESSION["security_number"]); 
	}
